...
現状: 完了扱い。詳細は `CURRENT_STATUS.md` を参照。
...
現状: かなり実装済み。ContentBlock の構造化、ツール結果表示、
SQLite FTS5 検索、検索 UI は入っている。残りは実ログでの精度向上と
大きな出力向け UI の磨き込み。
...
現状: 一部実装済み。複数ユーザーのスキャン、ユーザーフィルタ検索、
統計表示、単一パスワード認証、任意の watchdog 監視が入っている。
この archive では `CC_WATCH_ENABLED=false` の手動リロード運用を基本にする。
...
現状: 着手済み。セッション単位のメモ、タグ、お気に入り、
アーカイブ、review status、knowledge scope をローカル SQLite に保存し、
セッション詳細・一覧で表示できる。全セッションへの deterministic auto label
backfill も入っており、手動タグを壊さずにラベルを更新できる。
...
具体的な AGENT への受け渡し手順と UI は
`AGENT_CONTEXT_HANDOFF.md` を参照。Phase 4 の手動アノテーションだけでも、
最初の Agent Context Pack を生成できる設計にする。
